...
//    8.
    ...
    @Published public var objectOverlays: [ObjectOverlay] = []
    
    @Published public var image: CVPixelBuffer?
...
//    14
    ...
    private var modelDataHandler: ModelDataHandler? = ModelDataHandler(modelFileInfo: EfficientDet.modelInfo, labelsFileInfo: EfficientDet.labelsInfo)
    ...
    public func captureOutput(_ output: AVCaptureOutput, didOutput sampleBuffer: CMSampleBuffer, from connection: AVCaptureConnection) {
        ...
        result = self.modelDataHandler?.runModel(onFrame: imagePixelBuffer)
        ...
        DispatchQueue.main.async {
            self.refillResult(onInferences: displayResult.inferences, withImageSize: CGSize(width: CGFloat(width), height: CGFloat(height)))
            self.image = pixelBuffer
        }
    }
    
    func refillResult(onInferences inferences: [Inference], withImageSize imageSize:CGSize) {
        self.objectOverlays = []
        ...
        for inference in inferences {
            let objectOverlay = ObjectOverlay(confidence: inference.confidence, className: inference.className, rect: inference.rect, displayColor: inference.displayColor, imageSize: imageSize)
            
            self.objectOverlays.append(objectOverlay)
        }
    }
